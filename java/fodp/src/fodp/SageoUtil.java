...
public class SageoUtil {
	...
	public static double getQlPrime(double epsilonStar, double beta, double qr) {
		return 1.0 / qr * getQr(epsilonStar, beta);
	}

	public static double getQrPrime(double epsilon, double epsilonStar, double beta, double ql) {
		double qrPrime = -1;
		if (beta > 1 - Math.exp(-epsilon / 2)) {
			qrPrime = 1.0 / ql * getQl(epsilonStar, beta);
		} else {
			qrPrime = 0;
		}
		return qrPrime;
	...
	public static int getNu(double delta, double beta, double ql, double qr) {

		double A = ql / (1.0 - ql);
		double C = 1.0 / (1.0 - qr);

		double t = (delta * (A + C)) / (2.0 * beta + delta * A);

		double nu = Math.log(t) / Math.log(ql);
