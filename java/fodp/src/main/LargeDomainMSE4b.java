...
public class LargeDomainMSE4b {

	static int turnNum = 1;

	// Large-domain parameters
	static double sConstants[] = { 0.1, 1, 10 };

	public static void main(String[] args) {

		System.out
				.println("Method" + "\t" + "Dataset" + "\t" + "epsilon_E" + "\t" + "tau " + "\t" + "b" + "\t" + "MSE");
		int seed = 12345;
		...
		if (args.length == 2) {
			if (args[0].toLowerCase().equals("ageo")) {
				methods = new Method[] { Method.FOLNF_AGeo };
			} else if (args[0].toLowerCase().equals("1geo")) {
				methods = new Method[] { Method.FOLNF_1Geo };
			} else if (args[0].toLowerCase().equals("foud")) {
				methods = new Method[] { Method.FOUD };
			}
		...
		for (String dataName : dataNames) {
			...
			for (double sConstant : sConstants) {
