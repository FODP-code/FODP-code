...
public class TauOptimization {
	...
	public static void main(String[] args) {
		...
		Method methods[] = { Method.FOLNF_AGeo, Method.FOLNF_1Geo };
				...
				for (double delta_E : delta_Es) {
					...
					for (Method method : methods) {
						for (int tau : taus) {
							...
							int optimizedTau = TauGammaOptimizer.getOptimalTau(method, epsilon_E, delta_E, n, d, b);
							...
							System.out.println(getMethodName(method) + "\t" + dataName + "\t" + epsilon_E + "\t" + tau
									+ "\t" + meanTopK1 + "\t" + (tau == optimizedTau ? "(optimized tau)" : ""));
						}
					}
				}
			}
